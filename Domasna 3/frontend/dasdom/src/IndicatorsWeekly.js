...
const IndicatorsWeekly = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:8080/api/indicators/weekly')
    ...
    const handleFilter = () => {
        if (filterType === "id") {
            fetch(`http://localhost:8080/api/indicators/weekly/${filter}`)
                .then(response => response.json())
            ...
            fetch(`http://localhost:8080/api/indicators/weekly/symbol/${filter}`)
                .then(response => response.json())
    ...
    return (
        <div className="table-container">
            ...
            export default IndicatorsWeekly;
